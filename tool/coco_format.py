...
class YOLO:
    ...
    def save(self, data, img_path, img_type, manipast_path, file_name):
        ...
        with open(os.path.abspath(os.path.join(manipast_path, file_name)), "w") as manipast_file:
            ...
            for key in data:
                ...
                for idx in range(0, int(data[key]["objects"]["num_obj"])):
                    ...
                    xmin = str(int(data[key]["objects"][str(idx)]["bndbox"]["xmin"]))
                    ymin = str(int(data[key]["objects"][str(idx)]["bndbox"]["ymin"]))
                    xmax = str(int(data[key]["objects"][str(idx)]["bndbox"]["xmax"]))
                    ymax = str(int(data[key]["objects"][str(idx)]["bndbox"]["ymax"]))
                    cls_id = str(int(self.cls_list.index(data[key]["objects"][str(idx)]["name"])))

                    label += xmin + ' ' + ymin + ' ' + xmax + ' ' + ymax + ' ' + cls_id + ' '

                # Write file
                manipast_file.write(os.path.abspath(os.path.join(img_path, "".join([key, img_type, label, "\n"]))))
